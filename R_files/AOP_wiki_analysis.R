...
library(igraph)
par(bg="white") #background color setting for plots
...
b2gpal=colorRampPalette(c("black","green"))
...
## Barplot to show degree histogram
b2ppal=colorRampPalette(c("black","purple"))
barplot(table(degree(AOPg,mode="in")), xlab="In-Degree", ylab="# of KEs with In-Degree",col.axis="black", col.lab="black",col=b2ppal(max(degree(AOPg,mode="in"))))
legend('topright',legend=rev(seq(min(degree(AOPg,mode="in")),max(degree(AOPg,mode="in")),3)), pch=22,
       col="#777777", xjust=1,yjust=1, pt.bg=rev(b2ppal(length(rev(seq(min(degree(AOPg,mode="in")),max(degree(AOPg,mode="in")),3))))), pt.cex=2, cex=.8, bty="n", ncol=1, y.intersp=.5, box.col="white", text.col="black")
...
b2upal=colorRampPalette(c("black","blue"))
...
hist(log10(betweenness(AOPg)),breaks=20,col=b2upal(20), xlab=expression(paste("Log"[10],"(Betweeness)","")), main="")
## Barplot of betweenness totals with 20 bins
barplot(table(as.numeric(cut(betweenness(AOPg),breaks = 20))),col=b2upal(20))
...
b2mpal=colorRampPalette(c("black","magenta"))
...
b2cpal=colorRampPalette(c("black","cyan"))
...
barplot(table(eccentricity(AOPg,mode ="all")), xlab="Eccentricity", ylab="# of KEs with Eccentricity",col.axis="black", col.lab="black",col=b2cpal(max(eccentricity(AOPg,mode ="all"))))
legend('topright',legend=rev(seq(min(eccentricity(AOPg,mode ="all")),max(eccentricity(AOPg,mode ="all")),3)), pch=22,
       col="#777777", xjust=1,yjust=1, pt.bg=rev(b2cpal(length(rev(seq(min(eccentricity(AOPg,mode ="all")),max(eccentricity(AOPg,mode ="all")),3))))), pt.cex=2, cex=.8, bty="n", ncol=1, y.intersp=.5, box.col="white", text.col="black")
